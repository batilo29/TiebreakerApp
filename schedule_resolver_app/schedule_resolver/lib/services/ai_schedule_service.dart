...
class AiScheduleService extends ChangeNotifier {
  ...
  // IMPORTANT: Ensure you have a valid API key here
  ...
  Future<void> analyzeSchedule(List<TaskModel> task) async {
    ...
    try {
      // 1. Model Name: Ensure you use a valid model (e.g., 'gemini-1.5-flash')
      final model = GenerativeModel(model: 'gemini-1.5-flash', apiKey: _apiKey);
      final taskJson = jsonEncode(task.map((t) => t.toJson()).toList());

      // 2. Fixed String Interpolation: Added '$' before taskJson
      final prompt = '''
